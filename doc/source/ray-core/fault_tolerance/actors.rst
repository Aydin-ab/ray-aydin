...
which sets the maximum number of times that an actor is restarted.
The default value of ``max_restarts`` is 0, meaning that the actor isn't
restarted. If set to -1, the actor is restarted infinitely many times.
When an actor is restarted, its state is recreated by rerunning its
constructor.
After the specified number of restarts, subsequent actor methods
...
actor task is submitted to an actor that's unreachable, Ray will report the
error with ``RayActorError``, a Python-level exception that's thrown when
...
actor task is submitted to an actor that is unreachable, the system
automatically retries the task. With this option, the system only throws a
``RayActorError`` to the application if one of the following occurs: (1) the
actor's ``max_restarts`` limit has been exceeded and the actor can't be
...
task is submitted, this counts for one retry. The retry limit can be set to
...
For at-least-once actors, the system still guarantees execution ordering
according to the initial submission order. For example, any tasks submitted
after a failed actor task don't execute on the actor until the failed actor
task has been successfully retried. The system doesn't attempt to re-execute
...
.. note::
  ...
  only retries *incomplete* tasks. Previously completed tasks aren't
  re-executed.


At-least-once execution is best suited for read-only actors or actors with
ephemeral state that doesn't need to be rebuilt after a failure. For actors
that have critical state, the application is responsible for recovering the
state, for example, by taking periodic checkpoints and recovering from the checkpoint
upon actor restart.
...
.. note::
  ...
  on a different node.
  For example, save the checkpoint to cloud storage (such as S3) or a shared directory (for example, using NFS).
...
worker that created it and called ``ActorClass.remote()``. Similar to
...
Since :ref:`detached actors <actor-lifetimes>` don't have an owner, they're still restarted by Ray
even if their original creator dies. Detached actors continue to be
...
``ray.kill`` on any handle to the actor. Note that it doesn't need to be the
...
Note that by default, retrying on user raised exceptions is disabled. To enable it, make sure the method is **idempotent**, that's, invoking it multiple times should be equivalent to invoking it only once.
...
- ``False`` (default): No retries for user exceptions.

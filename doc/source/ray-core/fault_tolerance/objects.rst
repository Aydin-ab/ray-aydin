...
failures, Ray uses *lineage reconstruction* to recover the object.
Ray first automatically attempts to recover the value by looking
for copies of the same object on other nodes. If none are found, then Ray
automatically recovers the value by :ref:`re-executing <fault-tolerance-tasks>`
...
* The object, and any of its transitive dependencies, must have been generated
  ...
  ray.put aren't recoverable**.
* Tasks are assumed to be deterministic and idempotent. Thus,
  **by default, objects created by actor tasks aren't reconstructable**. To allow
...
* Tasks are only re-executed up to their maximum number of retries. By
  default, a non-actor task can be retried up to 3 times and an actor task
  can't be retried. This can be overridden with the ``max_retries`` parameter
...
lineage, set the environment variable ``RAY_max_lineage_bytes`` (default 1 GB)
...
setting, if there are no copies of an object left, an ``ObjectLostError`` is
raised.
...
Currently, **Ray doesn't support recovery from owner failure**. In this case, Ray
cleans up any remaining copies of the object's value to prevent a memory
leak. Any workers that subsequently try to get the object's value receive
an ``OwnerDiedError`` exception, which can be handled manually.

Understanding ``ObjectLostErrors``
----------------------------------

Ray throws an ``ObjectLostError`` to the application when an object can't be
...
- ``ObjectReconstructionFailedError``: This error is thrown if an object, or
  another object that this object depends on, can't be reconstructed due to
  one of the limitations described :ref:`previously
  <fault-tolerance-objects-reconstruction>`.
- ``ReferenceCountingAssertionError``: The object has already been deleted,
  so it can't be retrieved. Ray implements automatic memory management through
  distributed reference counting, so this error shouldn't happen in general.

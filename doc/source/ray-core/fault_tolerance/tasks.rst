...
Tasks can fail due to application-level errors, for example, Python-level exceptions,
or system-level failures, for example, a machine fails. This page describes the
...
return value. This wrapped exception is thrown to any worker that tries
...
because the process crashed or because the machine failed, Ray reruns
...
``RAY_TASK_MAX_RETRIES``. For example, by passing this to your driver script or by
...
When a task returns a result in the Ray object store, it's possible for the
resulting object to be lost **after** the original task has already finished.
In these cases, Ray also tries to automatically recover the object by
...
By default, Ray does **not** retry tasks upon exceptions thrown by application
code. However, you may control whether application-level errors are retried,
and even **which** application-level errors are retried, using the
...
exception, or pass a list of retry-able exceptions. An example is shown below.
...
returned by the task. By default, this sends a KeyboardInterrupt to the
task's worker if it's mid-execution. Passing ``force=True`` to ``ray.cancel``
force-exits the worker. See :func:`the API reference <ray.cancel>` for
``ray.cancel`` for more details.

Note that currently, Ray doesn't automatically retry tasks that have been
cancelled.

Sometimes, application-level code may cause memory leaks on a worker after
repeated task executions, for example, due to bugs in third-party libraries.
...
invocations of the given remote function, it automatically exits. By

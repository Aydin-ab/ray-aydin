...
When you spawn child processes from Ray workers, you are responsible for managing the lifetime of child processes. However, it's not always possible, especially when worker crashes and child processes are spawned from libraries (torch dataloader).

To avoid leaking user-spawned processes, Ray provides mechanisms to terminate all user-spawned processes when a worker that starts it exits. This feature prevents GPU memory leaks from child processes (for example, torch).

Ray provides following mechanisms to handle subprocess killing on worker exit:

- ``RAY_kill_child_processes_on_worker_exit`` (default ``true``): Only works on Linux. If true, the worker kills all *direct* child processes on exit. This won't work if the worker crashed. This isn't recursive, in that grandchild processes aren't killed by this mechanism.
...
- ``RAY_process_group_cleanup_enabled`` (default ``false``): If true (POSIX), Ray isolates each worker into its own process group at spawn and cleans up the worker's process group on worker exit using `killpg`. Processes that intentionally call `setsid()` detach and aren't killed by this cleanup.

On non-Linux platforms, subreaper isn't available. Per‑worker process groups are supported on POSIX platforms; on Windows, neither subreaper nor PGs apply. Users should manage child processes explicitly on platforms without support.

Note: The feature is meant to be a last resort to terminate orphaned processes. It's not a replacement for proper process management. Users should still manage the lifetime of their processes and clean up properly.
...
To enable the subreaper feature (deprecated), set using `_system_config` or equivalent cluster configuration at start. You must restart the cluster to apply the change. Prefer enabling `process_group_cleanup_enabled` instead.
...
Raylet maintains a list of "known" direct children pid it spawns, and when the Raylet process receives the SIGCHLD signal, it knows that one of its child processes (for example, the workers) has died, and maybe there are reparented orphan processes. Raylet lists all children pids (with ppid = raylet pid), and if a child pid isn't "known" (not in the list of direct children pids), Raylet thinks it's an orphan process and kills it using `SIGKILL`.

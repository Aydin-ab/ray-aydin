...
* **TUNE_DISABLE_AUTO_CALLBACK_LOGGERS**: Ray Tune automatically adds a CSV and
  ...
  `1` disables this automatic creation. Note that this most likely
  affects analyzing your results after the tuning run.
...
* **TUNE_DISABLE_DATED_SUBDIR**: Ray Tune automatically adds a date string to experiment
  directories when the name isn't specified explicitly or the trainable isn't passed
  as a string. Setting this environment variable to ``1`` disables adding these date strings.
* **TUNE_DISABLE_STRICT_METRIC_CHECKING**: When you report metrics to Tune through
  ``tune.report()`` and passed a ``metric`` parameter to ``Tuner()``, a scheduler,
  or a search algorithm, Tune errors
...
* **TUNE_FORCE_TRIAL_CLEANUP_S**: By default, Ray Tune gracefully terminates trials,
  ...
  Setting this variable to a non-zero, positive integer causes trials to be forcefully
...
* **TUNE_GLOBAL_CHECKPOINT_S**: Time in seconds that limits how often
  experiment state is checkpointed. If not set, this defaults to ``'auto'``.
  ``'auto'`` measures the time it takes to snapshot the experiment state
  and adjusts the period so that ~5% of the driver's time is spent on doing the snapshot.
...
* **TUNE_PLACEMENT_GROUP_PREFIX**: Prefix for placement groups created by Ray Tune. This prefix is used
  for example, to identify placement groups that should be cleaned up on start/stop of the tuning run. This is
  initialized to a unique name at the start of the first run.
* **TUNE_PLACEMENT_GROUP_RECON_INTERVAL**: How often to reconcile placement groups. Reconciliation is
  used to make sure that the number of requested placement groups and pending/running trials are in sync.
  In normal circumstances these shouldn't differ anyway, but reconciliation makes sure to capture cases when
  placement groups are manually destroyed. Reconciliation doesn't take much time, but it can add up when
...
* **TUNE_WARN_SLOW_EXPERIMENT_CHECKPOINT_SYNC_THRESHOLD_S**: Threshold for logging a warning if the experiment state syncing
  takes longer than this time in seconds. The experiment state files should be very lightweight, so this shouldn't take longer than ~5 seconds.
...
* **TUNE_RESTORE_RETRY_NUM**: The number of retries that are done before a particular trial's restore is determined
  unsuccessful. After that, the trial isn't restored to its previous checkpoint but rather from scratch.
  Default is ``0``. While this retry counter is taking effect, per trial failure number isn't incremented, which
...
* **TUNE_ONLY_STORE_CHECKPOINT_SCORE_ATTRIBUTE**:  If set to ``1``, only the metric defined by ``checkpoint_score_attribute``
  is stored with each ``Checkpoint``. As a result, ``Result.best_checkpoints`` contains only this metric,
  omitting others that would normally be included. This can significantly reduce memory usage, especially when many
  checkpoints are stored or when metrics are large. Defaults to ``0`` (that's all metrics are stored).
* **RAY_AIR_FULL_TRACEBACKS**: If set to 1, prints full tracebacks for training functions,

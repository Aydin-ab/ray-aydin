...
This tutorial walks you through the process of setting up a Tune experiment.
To get started, take a PyTorch model and learn how to leverage Ray Tune to
optimize the hyperparameters of this model.
Specifically, leverage early stopping and Bayesian Optimization through Hyperopt to do so.

.. tip:: If you have suggestions on how to improve this tutorial,
    `let Ray know <https://github.com/ray-project/ray/issues/new/choose>`_.
...
Then, define a simple PyTorch model to train.
...
In this example, use a small convolutional neural network consisting of one 2D convolutional layer, a fully
...
Below are implemented functions for training and evaluating your PyTorch model.
Define a ``train`` and a ``test`` function for that purpose.
...
Below, define a function that trains the PyTorch model for multiple epochs.
This function is executed on a separate :ref:`Ray Actor (process) <actor-guide>` underneath the hood,
so communicate the performance of the model back to Tune (which is on the main Python process).

To do this, call :func:`tune.report() <ray.tune.report>` in the training function,
...
Run one trial by calling :ref:`Tuner.fit <tune-run-ref>` and :ref:`randomly sample <tune-search-space>`
...
.. note:: Tune automatically runs parallel trials across all available cores/GPUs on your machine or cluster.
...
Integrate early stopping into the optimization process. Use :ref:`Adaptive Successive Halving Algorithm <tune-scheduler-hyperband>`, a scalable algorithm for `principled early stopping.`_

.. _`principled early stopping.`: https://blog.ml.cmu.edu/2018/12/12/massively-parallel-hyperparameter-optimization/

On a high level, Adaptive Successive Halving Algorithm (ASHA) terminates trials that are less promising and allocates more time and resources to more promising trials.
As the optimization process becomes more efficient, **increase the search space by 5x** by adjusting the parameter ``num_samples``.
...
* Browse the :ref:`gallery of examples <tune-examples-others>` to see how to use Tune with PyTorch, XGBoost, TensorFlow, etc.
* `Report issues or questions <https://github.com/ray-project/ray/issues>`__ by opening an issue on GitHub.
